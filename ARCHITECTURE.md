...
- **离线追赶**（每次 WS Connected，`catchUpRunning` 串行化防重连风暴并发）：① `SyncApi.pendingTasks(deviceId)` 拉本设备积压 pending 任务逐条 `executeTask`（复用 download/delete/mkdir 执行体）；② **扫描追赶**（对标 Android `catchUpFolder` 的 Phase2）——逐启用 folder `walk` 本地目录构建全量清单（目录项 + 文件项带 blake3，进程内 `hashCache` 按 path\|size\|mtime 缓存避免重连重算），`POST /sync/scan` 交服务端比对 trunk：**trunk 有本地无 → 补派 download**，补齐离线期间遗漏的下载。`triggerCatchUp(force)` 供 UI「重新对齐」手动触发。忽略 `.synctmp`/`.syncpending` 工作目录与 `.yt` 前缀的自用记账文件。
- **清单权威 = 基线台账，且台账与数据同生共死**（`sync/SyncBaseStore.ets`，详见 `harmony/README.md`）：picker 授权目录上 `fs.listFile` 不可信，所以清单以台账为准、磁盘遍历只作补充。台账写**沙盒 JSON + 公共目录镜像 `<同步>/.yt_baseline.json` 双份**——沙盒会被重装/清数据单独清掉而公共目录里的文件还在，只有沙盒一份时台账归零 → 清单为空 → 服务端**全量重派整个 folder**（即「鸿蒙一上线就全量拉一次」的根因；安卓不会，因为它的台账和数据同在外部存储、且清单直接来自磁盘遍历）。配套：每个 folder 根下哨兵 `.ytsync` 做**枚举自检**，清单为空且 `listFile` 列不出哨兵时**跳过本轮 scan**（宁可不报，绝不报空）；`flushNow()` 在 scan 前/`stop()`/`onBackground` 强制落盘，消除 1.5s 防抖丢账窗口。允许上报空清单的只有用户显式点「强制全量重取」。
