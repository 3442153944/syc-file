...
<template>
  <div class="monitor-network">
    ...
    <NGrid :cols="4" :x-gap="12" :y-gap="12" responsive="screen" item-responsive>
      <NGi span="2 m:1">
        <NCard size="small">
          <NStatistic label="↑ 上行速率" :value="fmtRate(network?.send_rate ?? 0)"/>
        </NCard>
      </NGi>
      <NGi span="2 m:1">
        <NCard size="small">
          <NStatistic label="↓ 下行速率" :value="fmtRate(network?.recv_rate ?? 0)"/>
        </NCard>
      </NGi>
      <NGi span="2 m:1">
        <NCard size="small">
          <NStatistic label="在线设备" :value="String(network?.online_devices ?? 0)"/>
        </NCard>
      </NGi>
      <NGi span="2 m:1">
        <NCard size="small">
          <NStatistic label="活动连接" :value="String(network?.active_connections ?? 0)"/>
        </NCard>
      </NGi>
    </NGrid>
