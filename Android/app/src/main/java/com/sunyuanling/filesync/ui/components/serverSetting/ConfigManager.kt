...
import com.sunyuanling.filesync.LogLevel
import java.io.File
...
object ConfigManager {
    ...
    fun save() {
        ...
        props["forceKeepAliveEnabled"] = AppConfig.forceKeepAliveEnabled.toString()
        ...
        props["routePath"] = AppConfig.routePath.trimEnd('/')
    ...
    fun load() {
        ...
        AppConfig.forceKeepAliveEnabled = props["forceKeepAliveEnabled"]?.toString()?.toBooleanStrictOrNull() ?: AppConfig.forceKeepAliveEnabled
        ...
        AppConfig.routePath = (props["routePath"] as? String ?: AppConfig.routePath.trimEnd('/'))
    }
    ...
    fun init() {
        try {
            ...
            // 没有权限就跳过，等权限申请完再初始化
            Log.w("ConfigManager", "配置文件读取失败，等待权限: ${e.message}")
        }
    }
}